...
def encode_cursor(*, published_at: datetime | None, item_id: int) -> str:
    normalized = _normalize_cursor_datetime(published_at)
...
def decode_cursor(cursor: str) -> tuple[datetime | None, int]:
    ...
    if not timestamp_text:
        return None, item_id
    published_at = datetime.fromisoformat(timestamp_text.replace("Z", "+00:00"))
    return _normalize_cursor_datetime(published_at), item_id
