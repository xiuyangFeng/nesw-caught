  ...
  <SectionCard title="模型额度审计控制台 (LLM Token Usage Console)" subtitle="系统运行过程中产生的 Token 消耗审计与成本估算">
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mt-2">
      ...
      <div class="rounded-xl border border-border/60 bg-white/[0.01] p-4 flex flex-col justify-between h-24 shadow-sm relative overflow-hidden group">
        ...
        <div class="text-xl font-bold text-text-faint font-mono tabular-nums mt-1 group-hover:text-text transition-colors">
          {{ stats.overall.total_tokens.toLocaleString() }}
        </div>
        <div class="num text-[10px] text-text-faint mt-1 flex justify-between">
          <span>In: {{ stats.overall.prompt_tokens.toLocaleString() }}</span>
          <span>Out: {{ stats.overall.completion_tokens.toLocaleString() }}</span>
        </div>
        <div class="absolute left-0 top-0 bottom-0 w-0.5 bg-system/80 shadow-[0_0_8px_var(--system)]" />
      </div>

      <!-- Metric 2: Real Cost (基于各模型配置单价换算) -->
      <div class="rounded-xl border border-border/60 bg-white/[0.01] p-4 flex flex-col justify-between h-24 shadow-sm relative overflow-hidden group">
        <div class="text-[10px] uppercase tracking-wider text-muted font-mono">Real Cost (USD)</div>
        <div class="text-xl font-bold text-success font-mono tabular-nums mt-1">
        ...
        <div class="absolute left-0 top-0 bottom-0 w-0.5 bg-success/80 shadow-[0_0_8px_var(--success)]" />
      </div>

      <!-- Metric 3: Active Models Stats -->
      <div class="rounded-xl border border-border/60 bg-white/[0.01] p-4 flex flex-col justify-between h-24 shadow-sm relative overflow-hidden group">
        <div class="text-[10px] uppercase tracking-wider text-muted font-mono">Active Providers</div>
        <div class="text-xl font-bold text-ai font-mono tabular-nums mt-1">
        ...
        <div class="absolute left-0 top-0 bottom-0 w-0.5 bg-ai/80 shadow-[0_0_8px_var(--ai)]" />
      </div>
      ...
      <div class="rounded-xl border border-border/60 bg-white/[0.01] p-4 flex flex-col justify-between h-24 shadow-sm relative overflow-hidden group">
        ...
        <div class="num text-[10px] text-text-faint mt-1">
          占比: {{ stats.overall?.total_tokens ? Math.round((stats.models?.[0]?.total_tokens || 0) / stats.overall.total_tokens * 100) : 0 }}%
        </div>
        <div class="absolute left-0 top-0 bottom-0 w-0.5 bg-warning/80 shadow-[0_0_8px_var(--warning)]" />
      </div>
    </div>
    ...
    <div
      ...
      :class="stats.budget?.over_budget
        ...
        : 'border-success/40 bg-success/[0.06] text-text-faint'"
    ...
    >
      <div class="flex items-center gap-2 font-mono tabular-nums">
        ...
        <span class="text-sm font-bold" :class="stats.budget?.over_budget ? 'text-danger' : 'text-success'">
      ...
      <span v-if="stats.budget?.over_budget" class="font-bold text-danger">⚠ 已超出月度预算</span>
      <span v-else class="text-success/80">预算内</span>
    </div>
    ...
    <div v-if="stats.models.length > 0" class="mt-4 border border-border/40 bg-black/10 rounded-xl p-3 overflow-hidden text-xs">
      ...
      <div class="space-y-2">
        <div v-for="m in stats.models" :key="m.model_name" class="flex items-center justify-between font-mono tabular-nums text-text-faint">
          ...
          <div class="flex gap-8 shrink-0">
            ...
            <span class="w-20 text-right" :class="m.cost_available ? 'text-success' : 'text-text-faint'">
