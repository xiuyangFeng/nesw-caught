...
<template>
  <article
    class="grid gap-3 rounded-[20px] border p-4 text-left transition duration-150 ease-out hover:-translate-y-px"
    :class="
      selected
        ? 'border-[#ffb66d] bg-[linear-gradient(160deg,rgba(35,23,11,0.98),rgba(18,13,10,0.98))] shadow-[0_18px_40px_rgba(255,159,47,0.16)]'
  ...
  >
    ...
    <div class="flex items-end justify-between gap-3">
      <strong class="text-[28px] leading-none text-text">{{ formatNumber(row.price) }}</strong>
      <span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="toneClass">
    ...
    <div class="flex items-center justify-between text-[11px] uppercase tracking-[0.12em] text-text-faint">
      <span>Vol {{ formatNumber(row.volume, 0) }}</span>
      <span>{{ row.is_abnormal ? 'Abnormal' : 'Stable' }}</span>
    </div>
  </article>
</template>
