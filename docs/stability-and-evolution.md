...
> **文档状态（2026-08-18）：** 第一节稳定性原则仍然有效。指标名称、失败处理和演进段落可能落后于当前实现；细节以代码和运维页为准。不要把后文中的“后续演进”自动当成当前任务。
...
通知能力统一基于 `notification_job` 表扩展，避免各通道直接读取业务表；投递侧提供 300 秒租约防并发重复投递、失败重试（最多 5 次，按退避序列）、`dedupe_key` 幂等去重。早期设想的独立 `signal_event` 表未采纳，已移除。
